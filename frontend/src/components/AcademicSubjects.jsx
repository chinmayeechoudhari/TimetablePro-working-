...
export default function AcademicSubjects() {
  ...
  const directory = useMemo(() => {
    ...
    const map = new Map()
    subjects
      .filter(s => s.group_id === selectedGroup.group_id)
      .forEach(s => {
        const key = s.subject_name.trim().toLowerCase()
        if (!map.has(key)) map.set(key, { name: s.subject_name, theory: null, lab: null })
        map.get(key)[s.subject_type] = s.periods_per_week
      })
    return [...map.values()]
      .map(row => ({
        ...row,
  ...
  const filterCounts = useMemo(() => ({
    ...
    theory: directory.filter(row => row.theory != null).length,
    lab: directory.filter(row => row.lab != null).length,
    'theory+lab': directory.filter(row => row.theory != null && row.lab != null).length,
  }), [directory])
  ...
  const filteredDirectory = useMemo(() => {
    ...
    return directory.filter(row => {
      ...
      const matchesType = typeFilter === 'all'
        || (typeFilter === 'theory' && row.theory != null)
        || (typeFilter === 'lab' && row.lab != null)
        || (typeFilter === 'theory+lab' && row.theory != null && row.lab != null)
      return matchesSearch && matchesType
    })
  }, [directory, search, typeFilter])
  ...
  const formModalContent = formModal && selectedGroup ? (
    <div className="subjects-modal-root">
      <div className="modal-backdrop" onMouseDown={e => e.target === e.currentTarget && closeFormModal()}>
        <div className="academic-modal subject-form-modal" role="dialog" aria-modal="true">
          <form onSubmit={submitForm}>
            <div className="subject-modal-header">
              <div className="subject-modal-context">
                <span>Subjects</span><i /><span>{selectedDepartment}</span><i /><span>{yearLabel(selectedGroup.year_of_study)}</span>
              </div>
              <button type="button" className="modal-close subject-modal-close" onClick={closeFormModal}>
              ...
              <div className="subject-modal-heading">
                ...
                <div>
                  ...
                  <h2>{formIsEdit ? 'Edit subject' : 'Add a new subject'}</h2>
            ...
            <div className="modal-actions subject-form-footer">
              ...
              <button className="primary-button modal-primary" disabled={formSaving}>
                <Icon name="check" size={17} />{formSaving ? 'Saving…' : (formIsEdit ? 'Save Changes' : 'Add Subject')}
              </button>
            </div>
          </form>
        </div>
  ...
  return (
    <div className="academic-page">
      ...
      {loadError && <div className="global-notice error">{loadError}</div>}
        ...
        <section className="department-picker-section">
          <div className="department-picker-intro">
            ...
            <div>
              ...
              <p>Choose a department below to manage its subjects year by year. Each department card gives a quick curriculum overview.</p>
            </div>
          </div>
          ...
          <div className="department-picker-tools">
            ...
            <div className="dept-search">
              ...
              <input value={departmentSearch} onChange={e => setDepartmentSearch(e.target.value)} placeholder="Search departments..." />
              {departmentSearch && <button onClick={() => setDepartmentSearch('')}><Icon name="close" size={13} /></button>}
            </div>
          </div>
                ...
                return (
                  <button className="department-picker-card" key={row.name} onClick={() => chooseDepartment(row.name)}>
                    <div className="department-card-heading">
                      <div className={`department-picker-logo avatar-accent-${accentIndex(row.name)}`}>
                        <span>{code}</span>
                        <Icon name="building" size={15} />
                      </div>
                      <span className="department-picker-arrow"><Icon name="arrowRight" size={16} /></span>
                    </div>
                    <div className="department-card-kicker">ACADEMIC DEPARTMENT</div>
                    <h3 className="department-picker-name">{row.name}</h3>
                    <div className="department-card-stats">
                      <span><b>{row.groups.length}</b> Years</span>
                      <span><b>{row.divisions}</b> Divisions</span>
                    </div>
                    <div className="department-card-subheading">CURRICULUM OVERVIEW</div>
                    <div className="department-year-summary">
                      {row.groups.map(g => {
                        ...
                        return (
                          <div className="department-year-summary-row" key={g.group_id}>
                            <div><span>{yearLabel(g.year_of_study)}</span><b>{g.divisions.length} division{g.divisions.length === 1 ? '' : 's'}</b></div>
                            <em>{count} subject{count === 1 ? '' : 's'} / division</em>
                          </div>
                        )
                      })}
                    </div>
                    <div className="department-card-footer">Open department <Icon name="arrowRight" size={14} /></div>
                  </button>
                )
              })}
            </div>
        ...
        <>
          <section className="subjects-context-bar">
            <div className="subjects-context-left">
              <div className={`subjects-context-avatar avatar-accent-${accentIndex(selectedDepartment)}`}>{(shortCodes[selectedDepartment] || deriveShortCode(selectedDepartment)).slice(0, 3)}</div>
              <div className="subjects-context-copy">
                <div className="section-kicker">SUBJECT CONFIGURATION</div>
                <h2>{selectedDepartment}</h2>
                <div className="subjects-context-meta"><span>{activeDeptRow.groups.length} Years</span><i /><span>{activeDeptRow.divisions} Divisions</span></div>
              </div>
            </div>
          ...
          <section className="subject-year-tabs">
            <div className="subject-year-tabs-heading">
              <div><div className="section-kicker">SELECT YEAR</div><p>Choose a year to view and manage its subjects.</p></div>
              <span>{directory.length} subject{directory.length === 1 ? '' : 's'} in current year</span>
            </div>
            <div className="subject-year-tab-list">
              {activeDeptRow.groups.map(g => (
                <button key={g.group_id} className={`subject-year-tab ${String(g.group_id) === String(selectedGroupId) ? 'active' : ''}`} onClick={() => chooseYear(g.group_id)}>
                  ...
                  <span><strong>{yearLabel(g.year_of_study)}</strong><small>{g.divisions.length} divisions · {new Set(subjects.filter(s => s.group_id === g.group_id).map(s => s.subject_name.trim().toLowerCase())).size} subjects</small></span>
                </button>
              ))}
            </div>
          </section>
          ...
          <section className="subject-directory-card">
            <div className="subject-directory-head">
              ...
              <div className="subject-directory-actions">
                <div className="dept-search subject-search-box">
                  <Icon name="search" size={16} />
            ...
            <div className="subject-filter-bar">
              <div className="subject-filter-label"><span>FILTER BY TYPE</span><small>{filteredDirectory.length} of {directory.length} shown</small></div>
              <div className="subject-filter-chips">
                {[
                  ['all', 'All', filterCounts.all],
                  ['theory', 'Theory', filterCounts.theory],
                  ['lab', 'Lab', filterCounts.lab],
                  ['theory+lab', 'Theory + Lab', filterCounts['theory+lab']],
                ].map(([value, label, count]) => (
                  <button key={value} className={`subject-filter-chip ${typeFilter === value ? `active ${value}` : ''}`} onClick={() => setTypeFilter(value)}>
            ...
            {directory.length === 0 ? (
              <div className="subject-empty">
                <div className="subject-empty-icon"><Icon name="book" size={20} /></div>
                <h3>No subjects configured yet</h3>
                <p>Start building the {yearLabel(selectedGroup.year_of_study)} curriculum for {selectedDepartment}.</p>
                <button className="primary-button" onClick={openAdd}>Add First Subject</button>
              </div>
            ) : filteredDirectory.length === 0 ? (
              <div className="subject-no-match"><div className="subject-empty-icon"><Icon name="search" size={19} /></div><strong>No subjects match these filters</strong><span>Try another type or clear the search.</span></div>
            ) : (
              <div className="subject-table-wrap">
                <div className="subject-table-header"><span>#</span><span>SUBJECT</span><span>TYPE</span><span>WEEKLY LOAD</span><span>DIVISIONS</span><span>ACTIONS</span></div>
                <div className="subject-list">
                  {filteredDirectory.map((row, index) => (
                    <div className="subject-card" key={row.name}>
                      ...
                      <div className="subject-card-load">{row.type === 'theory+lab' ? <><b>{row.theory}</b> theory + <b>{row.lab}</b> lab <span>/ week</span></> : <><b>{row.theory ?? row.lab}</b> periods <span>/ week</span></>}</div>
                      <div className="subject-card-divisions"><b>{selectedGroup.divisions.length}</b><span>{selectedGroup.divisions.map(d => d.division_name).join(' · ')}</span></div>
                      <div className="subject-card-actions"><button className="subject-icon-btn" onClick={() => openEdit(row)} title="Edit subject"><Icon name="edit" size={15} /></button><button className="subject-icon-btn danger" onClick={() => { setDeleteError(''); setDeleteTarget(row) }} title="Delete subject"><Icon name="trash" size={15} /></button></div>
                    </div>
                  ))}
                </div>
