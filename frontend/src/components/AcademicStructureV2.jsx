...
export default function AcademicStructureV2() {
  ...
  async function create(e) {
    ...
    if (!department.trim() || !academicYear.trim() || !yearConfigs.length) {
      setError('Enter the department, academic year, and at least one year with divisions.')
      return
    }
    try {
      const response = await axios.post(`${BASE}/academic-structure`, { academic_year: academicYear.trim(), department: department.trim(), years: yearConfigs })
  ...
  return <div className="academic-page">
    <section className="academic-hero">
      <div className="hero-copy">
        <div className="academic-eyebrow">ACADEMIC SCHEDULING</div>
        <h1>Academic Structure</h1>
        <p>Set up departments, years and divisions before subjects, faculty assignments and timetable generation.</p>
      </div>
      <div className="hero-metrics"><div><strong>{departments.length}</strong><span>Departments</span></div><i/><div><strong>{totalDivisions}</strong><span>Divisions</span></div></div>
    </section>
    ...
    {error && !modal && <div className="global-notice error">{error}<button onClick={() => setError('')}><Icon name="close" size={15}/></button></div>}
    ...
    <section className="department-table-wrap">
      ...
      {visible.length ? visible.map(d => <article className="department-row" key={d.name}>
        ...
        <div className="department-cell row-actions"><button title="Edit" onClick={() => { setEditName(d.name); setError(''); setModal({type:'edit',name:d.name}) }}><Icon name="edit" size={16}/></button><button className="danger" title="Remove" onClick={() => remove(d.name)}><Icon name="trash" size={16}/></button><button className="view-action" title="View" onClick={() => setSelectedDepartment(selectedDepartment === d.name ? '' : d.name)}><Icon name="chevron" size={19}/></button></div>
      </article>) : <div className="empty-departments"><div className="empty-icon"><Icon name="building" size={22}/></div><h3>No departments yet</h3><p>Create your first academic group to begin.</p><button className="create-group-button" onClick={() => setModal('create')}><Icon name="plus" size={17}/> Create academic group</button></div>}
    </section>
