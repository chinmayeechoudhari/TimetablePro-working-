...
def create_assignment(payload: AssignmentCreate, db: Session = Depends(get_db)):
    ...
    if not definition: raise HTTPException(status_code=404, detail="Subject not found.")
    if not payload.division_ids: raise HTTPException(status_code=400, detail="Select at least one division.")
    divisions = db.query(Division).filter(Division.division_id.in_(payload.division_ids)).all()
    if len(divisions) != len(set(payload.division_ids)): raise HTTPException(status_code=404, detail="One or more divisions were not found.")
    created = []
    for division in divisions:
        if division.group_id != definition.group_id: raise HTTPException(status_code=400, detail="Subject and division must belong to the same department/year.")
        subject = db.query(Subject).filter(Subject.class_id == division.class_id, Subject.subject_name == definition.subject_name, Subject.subject_type == definition.subject_type).first()
        if not subject:
            subject = Subject(subject_name=definition.subject_name, periods_per_week=definition.periods_per_week, subject_type=definition.subject_type, class_id=division.class_id)
            db.add(subject); db.flush()
        existing = db.query(TeachingAssignment).filter(TeachingAssignment.teacher_id == teacher.teacher_id, TeachingAssignment.definition_id == definition.definition_id, TeachingAssignment.division_id == division.division_id).first()
        if existing: continue
        link = db.query(TeacherSubject).filter(TeacherSubject.teacher_id == teacher.teacher_id, TeacherSubject.subject_id == subject.subject_id).first()
        if not link: db.add(TeacherSubject(teacher_id=teacher.teacher_id, subject_id=subject.subject_id))
        db.add(TeachingAssignment(teacher_id=teacher.teacher_id, definition_id=definition.definition_id, division_id=division.division_id, subject_id=subject.subject_id))
